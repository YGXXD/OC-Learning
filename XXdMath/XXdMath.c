...
simd_float3 xxd_simd_normalize(const simd_float3 v)
{
    return v * xxdInvSqrt(simd_length_squared(v));
}
...
simd_float4x4 xxdMakeViewLookAt(const simd_float3 eyePos, const simd_float3 focusPos, const simd_float3 up)
{
    simd_float3 z = xxd_simd_normalize(focusPos - eyePos);
    simd_float3 x = xxd_simd_normalize(simd_cross(up, z));
